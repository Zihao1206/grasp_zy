...
def convert(src_path, target_path):
    img_path = "{}/Images/zoneyung".format(src_path)
    label_path = "{}/Labels/zoneyung".format(src_path)
    ...
    for name in img_name:
        ...
        # 打开要写的label文件
        with open(os.path.join(target_path, name + ".txt"), 'w') as f:
            image_box = []
            # 打开要读的label文件
            with open(labelpath, 'r', encoding='utf-8-sig') as fl:
                class_label = 0
                image = name + ".jpg"
                height, width, _ = cv2.imread("{}/{}".format(img_path, image)).shape #480, 640
                for lines in fl:
                ...
                for box in xywha:
                    ...
                    f.write(str(x) + ' ' + str(y) + ' ' + str(w) + ' ' + str(h) + ' ' + str(
                        a) + ' ' + str(image_box[i][0]) + ' ' + str(image_box[i][1]) + ' '  + str(image_box[i][2]) + ' '+ str(image_box[i][3]) + ' ' + str(image_box[i][4])+ ' ' + str(image_box[i][5])+ ' ' + str(image_box[i][6])+ ' ' + str(image_box[i][7])+ '\n')
                    i = i+1
...
if __name__ == '__main__':
    convert("C:/Users/SZJX/Desktop/grasp-rectangle-labelling-master", "C:/grasp_static/zoneyung/labels")