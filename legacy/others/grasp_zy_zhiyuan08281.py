...
class Grasp:
    def __init__(self, hardware=False):
        ...
        if self.hardware:
            ...
            # self.lift2init_pose = [73, -129, 127, -0.7, 77, 1]
            ...
            self.lift2init_pose = [60, -129, 127, 30, 77, 1] ## 回转到初始状态
            self.lift2init_pose = [60, -129, 127, 30, 77, 1]
            # self.place_pose = [86, -129, 127, -0.7, 77, 62]
            self.place_mid_pose = [70, -129, 60, 30, 121, 1] ## 机械臂第三关节前倾
            # self.place_last_pose = [90, -69, 4.335, 0.9, 57.3, 62]
            self.place_mid_pose2 = [70, -129, 60, 0, 9, 1] ## 机械臂展开
            self.place_last_pose = [70, -104, 38, -2, 9, 1] ## 机械臂向下放置物体
        ...
        #     ])
        self.Tcam2base = np.array([
            [-0.06468978 ,-0.99757199 ,-0.02579469,0.21829451],
            [ 0.99776036 ,-0.06421803 ,-0.01871645,-0.01994438],
            [ 0.01701452 ,-0.02694768 , 0.99949204,0.03201678],
            [0., 0., 0., 1., ]
            ])
        ...
        model.load_state_dict(torch.load('test_251018_1004__zoneyung_/epoch_60_accuracy_1.00', map_location=self.device)) # scatch
        # 目标框检测模型加载
    ...
    def grasp_img2real_yolo(self, color_img, depth_img, grasp, slope_angle, topk, vis=False, color=(255, 0, 0),
        ...
        coordinate_cam = self.camera.get_coordinate(column+config.column1, row)
        ...
        xpc, ypc, zpc= np.linalg.inv(self.camera.intr) @ np.array([column+config.column1, row, 1]) * z
        # coordinate[2] -= 0.1
        ...
        # slope_angle = np.pi/5
        slope_angle = np.pi/8
        column_left, column_right = 60, 480
