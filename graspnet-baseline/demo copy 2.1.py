...
def get_and_process_data(data_dir):
    ...
    color = np.array(Image.open(os.path.join(data_dir, 'zy_5r.jpg')).convert('RGB'), dtype=np.float32) / 255.0
    depth = np.array(Image.open(os.path.join(data_dir, 'zy_5d.tiff')))
    # color = np.array(Image.open(os.path.join(data_dir, 'color.png')).convert('RGB'), dtype=np.float32) / 255.0
    # depth = np.array(Image.open(os.path.join(data_dir, 'depth.png')))
    workspace_mask = np.array(Image.open(os.path.join(data_dir, 'workspace_mask_0d.png')).convert('L'))
    ...
    workspace_mask_eroded[-border_size-20:, :] = 0  # 下边框
