...
from others.iou import iou
from others.show_bbox import draw_bbox
...
def nms(predicts: np.ndarray,
    ...
    # Filter predicts with low probability
    for i, predict in enumerate(predicts):
        if predict[0] < score_thresh or np.abs(predict[1]-predict[3]) > 280:
            vis[i] = True
            n_remainder -= 1
    ...
    while n_remainder > 0:
        ...
        # Append output
        output_predicts.append(max_p[1:5])
        output_indices.append(max_index)
        output_labels.append(max_p[5])
        vis[max_index] = True
        n_remainder -= 1

    return output_predicts, output_indices, np.array(output_labels, dtype=int)
